...
<div class="page-bar">
    <ul class="page-breadcrumb">
        ...
        <li>
            <a href="{{url('cortes')}}">Listado de cortes</a>
            <i class="fa fa-circle"></i>
        </li>
        <li>
            <span>Nuevo cortes</span>
        </li>
    </ul>
</div>
@endsection
...
<h3 class="page-title"> Nuevo cortes
</h3>
    ...
    <form method="post" action="" id="formulario" enctype="multipart/form-data">
        ...
        <div class="form-row">
            <div class="form-group col-md-12">
                ...
                <select class="js-example-basic-single" id="id_med" name="id_med" required>
                    <option></option>
                    @foreach($mediciones as $item)
                        <option value="{{$item->id}}">{{$item->direccion}}-{{$item->idCliente}}</option>
                    @endforeach
                </select>
            </div>
        </div>
			...
			$.ajax({
				...
				success: function( response ) {
					if (response.codigo==0) {
						...
						setTimeout(function(){window.location = "/lectorMedidorWeb/public/cortes"} , 2000);   
